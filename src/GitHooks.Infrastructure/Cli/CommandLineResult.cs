...
public sealed class CommandLineResult
{
    ...
    /// </summary>
    /// <param name="exception">The exception that occurred during process execution.</param>
    /// <returns>A new <see cref="CommandLineResult"/> with exit code 1 and the exception message as the error.</returns>
    public static CommandLineResult FromException(Exception exception)
    {
        ArgumentNullException.ThrowIfNull(exception);

        return new CommandLineResult(string.Empty, exception.Message, 1);
    }
}
